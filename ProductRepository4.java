...
//	private int totalCnt = 0;
    ...
    public void insert(Product p) throws AddException{
    	for(Product saveP :pList) {
    		//if(saveP.getProdNo().equals(p.getProdNo())){ 26 = 27
    		if(saveP.equals(p)) { //Product의 equals 메서드를쓴다 오버라이딩이 안되어있으니 object 클래스의 ==을 쓴다
    							  //따라서 사용자가 상품번호가 같다는 걸 equals메서드를 쓰고 싶으면 product내에 equals 메서드를 오버라이딩 해줘야한다
    			throw new AddException("이미존재하는 상품입니다");
    		}
    	}pList.add(p);
    }
